...
from src.dataset_preparation.TimeGAN.trainer.utils import make_tf_dataset, random_generator_tf  # batch_generator
...
def build_rnn_model(
    input_dim, output_dim, module_name, hidden_dim, num_layers, activation="sigmoid", name=None, bidirectional=False
):
    """Build Causal Keras RNN model with stacked cells.
    If bidrectional cerates bidirecitonal model"""
    cells = [rnn_cell(module_name, hidden_dim) for _ in range(num_layers)]
    if bidirectional:
        return tf.keras.Sequential(
            [
                tf.keras.layers.Input(shape=(None, input_dim)),
                tf.keras.layers.Bidirectional(tf.keras.layers.RNN(cells, return_sequences=True)),
...
def timegan(ori_data, parameters, checkpoint_dir):
    ...
    discriminator = build_rnn_model(
        hidden_dim, 1, module_name, hidden_dim, num_layers, activation=None, name="discriminator", bidirectional=True
    )
    ...
    def train_generator_embedder(X_mb, Z_mb, gamma=1.0):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            #     )
            # )
            # G_loss_V2 = tf.reduce_mean(tf.abs(tf.reduce_mean(X_hat_ng, axis=0) - tf.reduce_mean(X_mb, axis=0)))
            # G_loss_V = G_loss_V1 + G_loss_V2

            # Final losses
            # G_loss = G_loss_U + gamma * G_loss_U_e + 100.0 * tf.sqrt(G_loss_S) + 100.0 * G_loss_V
            G_loss = G_loss_U + 100 * tf.sqrt(G_loss_S) + gamma * G_loss_U_e
            E_loss = 10 * tf.sqrt(mse(X_mb, X_tilde)) + 0.1 * G_loss_S
        ...
        del tape
        return H, H_hat, E_hat, E_loss, G_loss, G_loss_S  # , G_loss_V
    ...
    def train_joint_step(X_mb, Z_mb, itt, gamma=1.0, threshold=0.15):
        H, H_hat, E_hat, E_loss, G_loss, G_loss_S = train_generator_embedder(X_mb, Z_mb, gamma)
        ...
        return D_loss, G_loss, G_loss_S, E_loss
    ...
    def save_timegan_models(epoch):
        embedder.save(os.path.join(checkpoint_dir, "embedder_epoch.keras"))
        recovery.save(os.path.join(checkpoint_dir, "recovery_epoch.keras"))
        generator.save(os.path.join(checkpoint_dir, "generator_epoch.keras"))
        supervisor.save(os.path.join(checkpoint_dir, "supervisor_epoch.keras"))
        discriminator.save(os.path.join(checkpoint_dir, "discriminator_epoch.keras"))

    # 1. Embedder training
    # print("Start Embedding Network Training")
    # for itt in tqdm(range(iterations), desc="Embedding Training"):
    #     X_mb, T_mb = next(iterator)
    #     step_e_loss = train_embedder(X_mb)
    #     if itt % 500 == 0:
    #         tqdm.write(f"Embedding step {itt}/{iterations}, E_loss: {step_e_loss.numpy():.4f}")
    ...
    # for itt in tqdm(range(iterations), desc="Supervisor Training"):
    #     X_mb, T_mb = next(iterator)
    #     Z_mb = random_generator_tf(batch_size, z_dim, T_mb, max_seq_len)
    #     step_g_loss_s = train_generator_supervised(Z_mb, X_mb)
    #     if itt % 500 == 0:
    #         tqdm.write(f"Supervised step {itt}/{iterations}, G_loss_S: {step_g_loss_s.numpy():.4f}")
    #         save_timegan_models(itt)
    # print("Finish Generator Supervised Training\n")

    # 3. Joint training
    print("Start Joint Training")
    for itt in tqdm(range(2 * iterations), desc="Joint Training"):
        X_mb, T_mb = next(iterator)
        # Generator + Embedder update twice, discriminator update one
        Z_mb = random_generator_tf(batch_size, z_dim, T_mb, max_seq_len)
        step_d_loss, step_g_loss_u, step_g_loss_s, step_e_loss_t0 = train_joint_step(X_mb, Z_mb, itt)
    ...
    # -----------------------------
    factor = 1
    new_array = np.tile(ori_time, factor)
    Z_mb = random_generator_tf(no * factor, z_dim, new_array, max_seq_len)
    H_hat = supervisor(generator(Z_mb, training=False), training=False)
    X_hat = recovery(H_hat, training=False).numpy()
    generated_data = [(X_hat[i, : new_array[i], :] * max_val + min_val) for i in range(no * factor)]
