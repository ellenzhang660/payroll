...
CONTEXT_LENGTH = 32  # multiple of 32
    ...
    def _prepare_samples(self) -> None:
        ...
        for idx in range(len(self.series)):
            self.series[idx] = np.concatenate(
                [np.zeros(max(0, self.context_length - len(self.series[idx]))), self.series[idx]]
            )
            for start_idx in range(0, len(self.series[idx]) - self.context_length + 1):
                ...
                if len(x_context) < self.context_length:
                    padding_length = self.context_length - len(x_context)
                    x_context = np.concatenate([x_context, np.zeros(padding_length)])
                    sys.exit()

                if len(x_future) < self.horizon_length:
                    padding_length = self.horizon_length - len(x_future)
...
def prepare_datasets(
    series: np.ndarray, context_length: int, horizon_length: int, freq_type: int = 0, train_split: float = 0.8
    ...
    train_dataset = TimeSeriesDataset(
        train_data, context_length=context_length, horizon_length=horizon_length, freq_type=freq_type
    )

    val_dataset = TimeSeriesDataset(
        val_data, context_length=context_length, horizon_length=horizon_length, freq_type=freq_type
    )
...
def get_model(from_dir: str, load_weights: bool = False):
    ...
    hparams = TimesFmHparams(
        ...
        horizon_len=128,
        num_layers=20,  # 20 for v1, 50 for v2
        use_positional_embedding=True,  # set to true for v1.0
        context_len=CONTEXT_LENGTH,  # Context length can be anything up to 2048 in multiples of 32
    )
    ...
    if load_weights:
        if from_dir:
            checkpoint_path = f"{from_dir}/torch_model.ckpt"
        else:
...
def get_data(context_len: int, horizon_len: int, freq_type: int = 0) -> Tuple[Dataset, Dataset]:
    ...
    synthetic_path = os.path.join(DATA_ROOT, FILE_PATH, "generated_data.pkl")
...
def single_gpu_example():
    ...
    train_dataset, val_dataset = get_data(
        context_len=CONTEXT_LENGTH, horizon_len=tfm_config.horizon_len, freq_type=config.freq_type
    )
